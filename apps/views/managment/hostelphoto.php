    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form role="form" action="<?=base_url('managment/hostelphoto/insert')?>" method="post" enctype="multipart/form-data">
            <div class="row">
            <div class="col-lg-12">
              <div class="form-group">
                <input type="text" name="hotelid" id="hotelid" value="<?=$hostel?>" hidden>
...
<?php  foreach ($hostelphoto->result_array() as $row){ ?>
<div id="condel<?=$row['hostel_photo_id']?>" class="modal fade">
  <div class="modal-dialog">
    <div class="modal-content">
      <form action="<?=base_url('managment/hostelphoto/delete')?>" method="post">
        <div class="modal-header">
          <input type="text" name="hotelid" id="hotelid" value="<?=$hostel?>" hidden>
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        ...
        <div class="modal-body">
          <p>ท่านต้องการลบข้อมูล <?=$row['hostel_photo_src']?> นี้ใช่หรือไม่ </p>
          <p><input type="text" name="delphoto" id="delphoto" value="<?=$row['hostel_photo_id']?>" hidden></p>
        </div>
        <div class="modal-footer">
  ...
  <div class="container">
    ...
    <div class="row">
      <table class="table table-bordered">
        ...
        <tbody>
         <?php 
         foreach ($hostelphoto->result_array() as $row){ ?>
         <tr>
          <td><img width="150" height="150" src="<?= base_url() ?><?=$row['hostel_photo_src']?>"></td>
          <td><button type="button" class="btn btn" style="background-color: #e84213;color: white;" onclick='$("#condel<?=$row['hostel_photo_id']?>").modal("show")'>ลบ</button></td>
        </tr>
        <?php } ?>
      </tbody>
    </table>
